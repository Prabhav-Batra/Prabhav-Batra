...
def get_skillicons_inner(icons):
    ...
    # Extract everything inside the main <svg ...> tag
    # The SVG ends with </svg>
    match = re.search(r'<svg[^>]*>(.*)</svg>', svg_data, re.DOTALL | re.IGNORECASE)
    if match:
...
svg_parts = [f'<svg width="100%" viewBox="0 0 {width} {height}" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">']
...
for title, icons in categories:
    ...
    # Fetch skillicons SVG inner contents
    icon_inner = get_skillicons_inner(icons)
    
    # Embed using a group with transform instead of nested SVG
    svg_parts.append(f'<g transform="translate(370, {y_offset + 90})">{icon_inner}</g>')
    
    y_offset += 190

svg_parts.append('</svg>')

with open('assets/images/TechArsenal_v2.svg', 'w') as f:
    f.write('\n'.join(svg_parts))

print("TechArsenal_v2.svg generated successfully!")
